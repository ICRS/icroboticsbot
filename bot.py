...
async def on_message(message):
    ...
    if message.channel.id == FILE_CHANNEL and message.attachments:
        ...
        for attachment in message.attachments:
            if (
                attachment.filename.split(".")[-1].lower() in extension_list
        ...
        await admin.send(
            f'{message.author} sent {len(files)} files with names {[file["name"] for file in files]}'
        )
    ...
    if message.content.startswith("!quote"):
        ...
        await message.channel.send(
            file=discord.File("/home/pi/code/icroboticsbot/quote.png")
        )
                    ...
                    if member:
                        if not shortcode_exists(shortcode):
                            ...
                            await member.add_roles(
                                role, reason="Membership verified by roboticsbotbot"
                            )
                            print("added role")
                            add_mapping(shortcode, member.id)
                            await message.channel.send("Membership verified \nEnjoy!")
                            await admin.send(
                                "Bot responded: Membership verified \nEnjoy!"
                            )
                            ...
                            if valid:
                                await message.channel.send(
                                    "Someone has already verified using this shortcode. \nIf this is not you, message a committee member"
                                )
                                await admin.send(
                                    "Bot responded: Someone has already verified using this shortcode. \nIf this is not you, message a committee member"
                                )
                            else:
                                #   flip valid
                                change_valid(member.id, 1)
                                await message.channel.send(
                                    "Membership reverified \nWelcome back!"
                                )
                                await admin.send(
                                    "Bot responded: Membership reverified \nWelcome back!"
                                )
                                pass
                    else:
                        await message.channel.send(
                            "Maybe try joining the ICRS discord server first?"
                        )
                else:
                    await message.channel.send("""
