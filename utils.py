...
def random_quote(author):
    ...
    backgrounds = [
        "/home/pi/code/icroboticsbot/background_images/" + image
        for image in images
        if image.startswith(author.strip().lower())
    ]
    ...
    choices = [
        quote for quote in quotes if quote["author"].lower() == author.strip().lower()
    ]
