...
async def register_user(interaction: discord.Interaction, *, shortcode: str):
    ...
    try:
        ...
        if not membershipPaid.json():
            return await interaction.response.send_message(
                embed=discord.Embed(
                    title="You have not paid for lab pass",
                    description="Please pay £5 for lab pass before trying again\n here a link: <https://www.imperialcollegeunion.org/shop/csp/robotics/lab-pass>",  # noqa: E501
                    color=discord.Color.red()
                ),
                ephemeral=True
            )
