...
async def add_role_and_update(server, member, author, ctx):
        logging.info(f"Adding mapping (role and update): {member, type(member), author, type(author)}")
        role = discord.utils.get(server.roles, name='Verified Member')
        await member.add_roles(role, reason="Membership verified using API")
        logging.info(f"Adding mapping (role and update): {member, type(member), author, type(author)}")
        add_mapping(shortcode, author)
...
async def send_error_msg(ctx, e):
